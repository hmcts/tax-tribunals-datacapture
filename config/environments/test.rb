...
Rails.application.configure do
  ...
  # Deployed smoke tests only drive the service through Capybara. Avoid the
  # schema checks and eager loading that would otherwise require DB credentials.
  database_less_smoke_test = ENV["SKIP_TEST_DATABASE"] == "true"
  config.active_record.maintain_test_schema = false if database_less_smoke_test

  # Eager loading loads your entire application. When running a single test locally,
  # this is usually not necessary, and can slow down your test suite. However, it's
  # recommended that you enable it in continuous integration systems to ensure eager
  # loading is working properly before deploying your code.
  config.eager_load = ENV["CI"].present? && !database_less_smoke_test
