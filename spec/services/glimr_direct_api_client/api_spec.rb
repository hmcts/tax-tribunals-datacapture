...
RSpec.describe GlimrDirectApiClient::Api, focus: true do
  ...
  describe '#post' do
    ...
    context 'glimr errors' do
      context 'with :glimererrorcode' do
        ...
        it 'raises Unavailable with the glimr error message' do
          expect { subject.post }.to raise_error(GlimrDirectApiClient::Unavailable, 'Some Glimr Error')
        end
      end
      ...
      context 'without :glimrerrorcode' do
        ...
        it 'raises Unavailable with the glimr error message' do
          expect { subject.post }.to raise_error(GlimrDirectApiClient::Unavailable, 'Error without code')
        end
      end
    end
    ...
    context 'network errors' do
      it 'raises Unavailable on 404' do
        ...
        expect { subject.post }.to raise_error(GlimrDirectApiClient::Unavailable, '404')
      end

      it 'raises Unavailable when it receives a 400' do
        stub_request(:post, /endpoint$/).to_return(status: 400)
        expect { subject.post }.to raise_error(GlimrDirectApiClient::Unavailable, '400')
      end

      it 'raises Unavailable when it receives a 500' do
        stub_request(:post, /endpoint$/).to_return(status: 500)
        expect { subject.post }.to raise_error(GlimrDirectApiClient::Unavailable, '500')
      end

      it 'raises Unavailable when it receives a 599' do
        stub_request(:post, /endpoint$/).to_return(status: 599)
        expect { subject.post }.to raise_error(GlimrDirectApiClient::Unavailable, '599')
      end
