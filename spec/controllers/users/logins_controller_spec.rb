...
RSpec.describe Users::LoginsController do
  ...
  context 'with TAX_APPEAL intent' do
    before do
      allow(subject).to receive(:current_tribunal_case).and_return(tribunal_case_appeal)
    end
    ...
    describe '#create' do
      ...
      context 'when the authentication was successful' do
        ...
        context 'when the case is new and we are linking it to the user (we send an email)' do
          before do
            expect(TaxTribs::SaveCaseForLater).to receive(:new).with(tribunal_case_appeal, user).and_return(double(save: true, email_sent?: true))
          end
  ...
  context 'with CLOSE_ENQUIRY intent' do
    before do
      allow(subject).to receive(:current_tribunal_case).and_return(tribunal_case_closure)
    end
    ...
    describe '#create' do
      ...
      context 'when the authentication was successful' do
        it 'signs the user in and takes to closure case type page' do
          do_post
          expect(response).to redirect_to(users_cases_path)
        end
        ...
        context 'when the case is new and we are linking it to the user (we send an email)' do
          before do
            expect(TaxTribs::SaveCaseForLater).to receive(:new).with(tribunal_case_closure, user).and_return(double(save: true, email_sent?: true))
          end
